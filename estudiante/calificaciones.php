...
$sql_materias = "SELECT i.inscripcion_id, m.nombre AS materia, m.nivel, c.nombre AS ciclo,
                        u.nombre AS prof_nombre, u.apellido_paterno AS prof_apellido_p, u.apellido_materno AS prof_apellido_m
        ...
        <?php if($materia_actual): ?>
            <div class="page-title-center">
                ...
                <p style="color: var(--text-dark); font-weight: 500; margin-top: 10px;">
                    <i class="fas fa-chalkboard-teacher" style="color:#aaa;"></i> Profesor: <?php 
                        // Usar paterno + materno si existen, de lo contrario solo nombre
                        $nombreProf = $materia_actual['prof_nombre'];
                        if (!empty($materia_actual['prof_apellido_p'])) {
                            $nombreProf .= ' ' . $materia_actual['prof_apellido_p'];
                            if (!empty($materia_actual['prof_apellido_m'])) {
                                $nombreProf .= ' ' . $materia_actual['prof_apellido_m'];
                            }
                        }
                        echo htmlspecialchars($nombreProf);
                    ?>
                </p>
