...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['foto_perfil'])) {
    ...
    // Validación: Revisar si la imagen es muy pesada o hubo error al subir
    ...
    // c) Renombrar el archivo (Aleatorio y forzamos SIEMPRE a extensión .webp)
    $new_file_name = bin2hex(random_bytes(10)) . '.webp';
    $target_file = $target_dir . $new_file_name;

    // d) RE-ENCODIZAR LA IMAGEN (Abre el archivo original según su tipo)
    ...
    // Guardamos la nueva copia limpia, SIEMPRE convertida a WebP
    if ($img_source) {
        // imagewebp recibe la imagen, la ruta, y la calidad (80 es excelente balance)
        ...
        // Borrar foto vieja si existe para no acumular basura
        if($old_foto && file_exists($target_dir . $old_foto)) {
            unlink($target_dir . $old_foto);
        }

        // Guardar nuevo nombre en la BD
