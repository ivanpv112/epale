...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["archivo_csv"])) {
    ...
    if ($_FILES["archivo_csv"]["size"] == 0) { header("Location: vista_csv_diagnosticos?msg=error_file"); exit; }
    ...
    try {
        ...
        header("Location: vista_csv_diagnosticos?msg=ok_diagnosticos&total=$registros_exitosos"); 
        exit;
        
    } catch (Exception $e) {
        if ($pdo->inTransaction()) $pdo->rollBack(); 
        if (isset($file)) fclose($file); 
        header("Location: vista_csv_diagnosticos?msg=error_db&fila=$fila_actual"); 
        exit;
    }
} else { 
    header("Location: vista_csv_diagnosticos"); 
    exit; 
}
?>