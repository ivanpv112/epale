...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["archivo_csv"])) {
    ...
    try {
        ...
        $stmt_user = $pdo->prepare("INSERT INTO usuarios (codigo, nombre, apellido_paterno, apellido_materno, correo, password, rol, estatus, genero, periodo_ingreso) VALUES (?, ?, ?, ?, ?, ?, 'PROFESOR', 'ACTIVO', ?, ?)");
        $stmt_prof = $pdo->prepare("INSERT INTO profesores (usuario_id, nacionalidad, experiencia) VALUES (?, ?, ?)");
        ...
        while (($datos = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            $genero = !empty($datos[6]) && in_array(strtoupper($datos[6]), ['MASCULINO', 'FEMENINO', 'OTRO']) ? strtoupper($datos[6]) : null;
            ...
            $hash_password = password_hash("PROFE" . substr($codigo, -6), PASSWORD_DEFAULT);
            
            try {
                $stmt_user->execute([$codigo, $nombre, $apellido_paterno, $apellido_materno, $correo, $hash_password, $genero, $periodo]);
                $stmt_prof->execute([$pdo->lastInsertId(), $nacionalidad, $experiencia]);
            } catch (PDOException $e) {
                if ($e->errorInfo[1] == 1062) { $pdo->rollBack(); fclose($file); header("Location: vista_csv_profesores.php?msg=dup&codigo=" . urlencode($codigo) . "&fila=$fila_actual"); exit; } throw $e; 
            }
