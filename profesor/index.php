...
// 1. OBTENER GRUPOS ACTIVOS Y CONTEO DE ALUMNOS (SQL Corregido para modo estricto WAMP)
$sql_grupos = "SELECT m.nombre AS materia, m.nivel, c.nombre AS ciclo, g.materia_id, g.ciclo_id, g.profesor_id,
                      ...
                      (SELECT COUNT(DISTINCT i.alumno_id) 
                       FROM inscripciones i JOIN grupos g2 ON i.nrc = g2.nrc 
               ...
               GROUP BY g.materia_id, c.ciclo_id, g.profesor_id, m.nombre, m.nivel, c.nombre
...
foreach ($grupos as $g) {
    $total_alumnos += $g['inscritos'];
}

// 3. DETERMINAR QUÉ CLASES TIENE "HOY"
$dia_num = date('N'); // 1=Lunes, ..., 5=Viernes
$letra_hoy = '';
switch($dia_num) {
    case 1: $letra_hoy = 'L'; break;
    case 2: $letra_hoy = 'M'; break;
    case 3: $letra_hoy = 'I'; break; // Usaremos I, X o W para Miércoles en la lógica
    case 4: $letra_hoy = 'J'; break;
    case 5: $letra_hoy = 'V'; break;
}

$clases_hoy = [];
if ($letra_hoy !== '') {
    foreach ($grupos as $g) {
        // Verificar presencial
        if ($g['dias_p']) {
            $dias_p = strtoupper($g['dias_p']);
            if (strpos($dias_p, $letra_hoy) !== false || ($letra_hoy == 'I' && (strpos($dias_p, 'MIE') !== false || strpos($dias_p, 'X') !== false))) {
                $clases_hoy[] = [
                    'materia' => $g['materia'] . ' ' . $g['nivel'],
                    'aula' => $g['aula_p'] ?: 'Sin Aula',
                    'inicio' => $g['inicio_p'],
                    'fin' => $g['fin_p'],
                    'tipo' => 'Presencial'
                ];
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="dash-header">
            ...
            <p>Prof. <?php echo htmlspecialchars($nombre_profesor); ?></p>
        </div>
        ...
        <div class="stats-row">
            <div class="stat-box">
                ...
                <div>
                    <div class="number"><?php echo $total_alumnos; ?></div>
            ...
            <div class="stat-box">
                ...
                <div>
                    <div class="number">3</div> <div class="label">Tareas Pendientes</div>
                </div>
            </div>
        </div>
        ...
        <div class="main-grid">
                    ...
                    <tbody>
                        <?php if($total_grupos > 0): ?>
                            <?php foreach($grupos as $g): 
                                // Para mostrar un horario resumido en la tabla
                                $horario_resumen = '---';
                                $aula_resumen = '---';
                                if($g['dias_p']) { 
                                    $horario_resumen = $g['dias_p'] . ' ' . date('H:i', strtotime($g['inicio_p'])) . '-' . date('H:i', strtotime($g['fin_p']));
                                    $aula_resumen = $g['aula_p'];
                                } elseif($g['dias_v']) {
                                    $horario_resumen = $g['dias_v'] . ' ' . date('H:i', strtotime($g['inicio_v'])) . '-' . date('H:i', strtotime($g['fin_v']));
                                    $aula_resumen = $g['aula_v'] ?: 'Virtual';
                                }
                            ?>
                                <tr onclick="window.location.href='detalle_grupo.php?mat=<?php echo $g['materia_id']; ?>&ciclo=<?php echo $g['ciclo_id']; ?>'">
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="5" style="text-align: center; color:#aaa; padding: 30px;">No tienes grupos asignados en el ciclo activo.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <div class="content-card">
                ...
                <div class="task-item task-urgent">
                    <div class="task-title">Calificar Quizzes - Inglés IV</div>
                    <div class="task-meta">Fecha límite: Hoy a las 23:59</div>
                </div>
                
                <div class="task-item">
                    <div class="task-title">Subir calificaciones parciales - Francés I</div>
                    <div class="task-meta">Fecha límite: 25 Feb 2026</div>
                </div>

                <div class="task-item">
                    <div class="task-title">Revisión de Writing Projects - Inglés IV</div>
                    <div class="task-meta">Fecha límite: 28 Feb 2026</div>
                </div>
                
                <div style="text-align: center; margin-top: 15px;">
                    <a href="#" style="color: var(--udg-blue); text-decoration: none; font-size: 0.9rem; font-weight: bold;">Ver todas las tareas <i class="fas fa-arrow-right"></i></a>
                </div>
            </div>
                ...
                <div class="today-classes">
                    <?php foreach($clases_hoy as $c): ?>
                        <div class="class-card">
                            <div>
                                <div class="class-name"><?php echo htmlspecialchars($c['materia']); ?></div>
                                <div class="class-room">
                                    <?php echo ($c['tipo'] == 'Presencial') ? '<i class="fas fa-building" style="color:#28a745;"></i>' : '<i class="fas fa-laptop-house" style="color:#17a2b8;"></i>'; ?>
                                    <?php echo htmlspecialchars($c['aula']); ?>
                                </div>
                            </div>
                            <div class="class-time">
                                <?php echo date('H:i', strtotime($c['inicio'])) . ' - ' . date('H:i', strtotime($c['fin'])); ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
            <?php else: ?>
                <p style="color: #888; margin: 0; padding: 15px 0;"><i class="fas fa-coffee" style="margin-right: 8px;"></i>No tienes clases programadas para el día de hoy. ¡Disfruta tu día!</p>
            <?php endif; ?>
        </div>
