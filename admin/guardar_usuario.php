...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        if (!empty($id)) {
            // --- ACTUALIZAR ---
            $sql = "UPDATE usuarios SET nombre=?, apellido_paterno=?, apellido_materno=?, correo=?, telefono=?, rol=?, estatus=?, codigo=? WHERE usuario_id=?";
            $params = [$nombre, $apellido_paterno, $apellido_materno, $correo, $telefono, $rol, $estatus, $codigo, $id];
            
            if (!empty($password)) {
                $sql = "UPDATE usuarios SET nombre=?, apellido_paterno=?, apellido_materno=?, correo=?, telefono=?, rol=?, estatus=?, codigo=?, password=? WHERE usuario_id=?";
                $params = [$nombre, $apellido_paterno, $apellido_materno, $correo, $telefono, $rol, $estatus, $codigo, password_hash($password, PASSWORD_DEFAULT), $id];
            }
            ...
            $sql = "INSERT INTO usuarios (nombre, apellido_paterno, apellido_materno, correo, password, telefono, rol, estatus, codigo) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$nombre, $apellido_paterno, $apellido_materno, $correo, password_hash($password, PASSWORD_DEFAULT), $telefono, $rol, $estatus, $codigo]);
